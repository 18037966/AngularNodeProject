...
export class PostsService {
  ...
  getPosts(postsPerPage: number, currentPage: number){
      ...
      this.http.get<{message: string, posts: any, maxPosts: number}>('http://localhost:3000/api/posts' + queryParams)
          .pipe(map((postData) => {
             ...
             }), maxPosts: postData.maxPosts};//this pipe is to convert the _id of database value to proper id.
          }))
          .subscribe((transformedPostData) => {
            this.posts = transformedPostData.posts;
            this.postsUpdated.next({posts: [...this.posts], postCount: transformedPostData.maxPosts});//pass a copy of the post so that we cannot edit the post in the service
          });
  ...
  addPost(title: string, content: string, image: File){
    ...
    const postData = new FormData(); //postdata allows us to compute files as well
    postData.append("title", title);
    postData.append("content", content);
    postData.append("image", image, title);
